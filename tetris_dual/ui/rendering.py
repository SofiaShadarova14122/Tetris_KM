...
def draw_next_piece_preview(board, center_x, top_y, dim=False):
    ...
    shape_h = len(shape)
    shape_w = len(shape[0])
    start_x = center_x - (shape_w * PREVIEW_CELL_SIZE) // 2
    arcade.draw_text("Следующая:", center_x, top_y + 15, TEXT_COLOR, 12, anchor_x="center")
...
def draw_scores_under_boards(score1, score2, center_x1, y1, center_x2, y2):
    arcade.draw_text(f"Игрок 1: {score1}", center_x1, y1, TEXT_COLOR, 18, anchor_x="center")
    arcade.draw_text(f"Игрок 2: {score2}", center_x2, y2, TEXT_COLOR, 18, anchor_x="center")

def draw_controls_bottom(screen_width, screen_height):
    y = 35
    arcade.draw_text("Управление:", screen_width//2, y+30, TEXT_COLOR, 14, anchor_x="center")
    arcade.draw_text("Игрок 1: A/D — влево/вправо, W — поворот, S — ускорение",
                     screen_width//2, y, TEXT_COLOR, 12, anchor_x="center")
    arcade.draw_text("Игрок 2: ←/→ — влево/вправо, ↑ — поворот, ↓ — ускорение",
                     screen_width//2, y-18, TEXT_COLOR, 12, anchor_x="center")
    arcade.draw_text("Пробел или ESC — пауза", screen_width//2, y-36, TEXT_COLOR, 12, anchor_x="center")
