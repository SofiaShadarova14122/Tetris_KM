...
def play_drop():
    if not _sounds_enabled:
        return
    global _drop_sound
    if _drop_sound is None:
        _drop_sound = _load_sound(SOUND_DROP)
    if _drop_sound:
        arcade.play_sound(_drop_sound, volume=0.5)

def play_rotate():
    if not _sounds_enabled:
        return
    global _rotate_sound
    if _rotate_sound is None:
        _rotate_sound = _load_sound(SOUND_ROTATE)
    if _rotate_sound:
        arcade.play_sound(_rotate_sound, volume=0.4)

def play_line_clear():
    if not _sounds_enabled:
        return
    global _line_clear_sound
    if _line_clear_sound is None:
        _line_clear_sound = _load_sound(SOUND_LINE_CLEAR)
    if _line_clear_sound:
        arcade.play_sound(_line_clear_sound, volume=0.6)