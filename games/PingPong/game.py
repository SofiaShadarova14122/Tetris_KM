...
class Ball:
    def __init__(self):
        ...
        self.y = 475
        self.radius = 12
        self.base_speed = 380  # Начальная скорость
    ...
    def draw(self):
        # Мяч рисуется всегда (в том числе до запуска)
        arcade.draw_circle_filled(self.x, self.y, self.radius, (255, 255, 255))
        # Лёгкая тень для объёма
        arcade.draw_circle_filled(self.x + 2, self.y - 2, self.radius, (220, 220, 220))

    def reset(self, direction):
        """direction: 1 = подача вверх, -1 = подача вниз"""
        self.speed = self.base_speed  # ✅ Сброс скорости к начальной
        ...
        self.active = False
        # X и Y синхронизируются с ракеткой в game.update()
    ...
    def _bounce(self, paddle):
        ...
        # ✅ ПОСТЕПЕННОЕ УСКОРЕНИЕ В ТЕЧЕНИЕ РАУНДА
        self.speed = min(self.speed * 1.09, 1100)
...
class PingPongGame:
        ...
        if self.ball.active:
            ...
            if result == "p1_missed":
                ...
                if not self.game_over:
                    self.ball.reset(direction=-1)  # P2 подаёт вниз
                    self.awaiting_start = True
                ...
                self._check_win()
                if not self.game_over:
                    self.ball.reset(direction=1)  # P1 подаёт вверх
                    self.awaiting_start = True
                    self.rounds_played += 1
    ...
    def draw(self):
        ...
        # ✅ СЧЁТ НА НИЖНЕМ СЛОЕ (рисуется первым)
        try:
            # Приглушённые цвета, соответствующие игрокам
        ...
        # МЯЧ (ВЕРХНИЙ СЛОЙ, не перекрывается счётом)
        self.ball.draw()
