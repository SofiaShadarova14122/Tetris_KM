...
class FishingWindow(arcade.Window):
    def __init__(self, bear, mode=None):
        ...
        self.center_window()  # Просто центрируем

        self.bear = bear
        self.kb = InputManager()
        self.kb.clear()
        if self.bear:
            self.bear.clear_queue()
    ...
    def on_update(self, dt):
        ...
        if self.bear:
            for p, a in self.bear.get_actions():
                self.kb.handle_bear(p, a)
    ...
    def on_key_press(self, k, m):
        ...
        elif k == arcade.key.ENTER and self.game.show_rules:
            self.game.start()
        elif k in (arcade.key.ESCAPE, arcade.key.M):
            arcade.close_window()
